...
- (void)timeRepeat{
    ...
    if (index%2 == 0) {
        [UIView animateWithDuration:1 animations:^{
            self.oneLabel.frame = CGRectMake(margin, 0, kWidth , kHeight);
            self.twoLabel.frame = CGRectMake(margin, -kHeight, kWidth , kHeight);
        } completion:^(BOOL finished){
            self.twoLabel.frame = CGRectMake(margin, kHeight, kWidth, kHeight);
        }];
    }else{
        [UIView animateWithDuration:1 animations:^{
            self.twoLabel.frame = CGRectMake(margin, 0, kWidth, kHeight);
            self.oneLabel.frame = CGRectMake(margin, -kHeight, kWidth, kHeight);
        } completion:^(BOOL finished){
            self.oneLabel.frame = CGRectMake(margin, kHeight, kWidth, kHeight);
        }];
    }
...
- (void)layoutSubviews{
    ...
    if (self.isHaveHeadImg) {
        ...
        self.headImageView.frame = CGRectMake(0, 0, kHeight,kHeight);
        margin = kHeight +10;
    }else{
